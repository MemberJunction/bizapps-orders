...
export class mjBizAppsOrdersProductCategory_ {
    ...
    @Field(() => Boolean, {nullable: true, description: `Whether products in this category are taxable (D73). NULL means inherit from the product type. Taxability resolves product -> category -> type, most specific wins.`}) 
    IsTaxable?: boolean;
        
    @Field({nullable: true, description: `Default taxability key for products in this category, matched against accounting's TaxRate.TaxCategory. NULL means inherit from the product type.`}) 
    @MaxLength(50)
    TaxCategory?: string;
