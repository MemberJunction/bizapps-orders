...
> Derived from the plan chain @ `2aa2ea2` (2026-07-15) · MODs through MOD-11 · UPDs through UPD-6
...
| B.3 | Coupons / promo codes (Coupon + CouponRedemption schema S7 + F10 engine) | Planned — gated on Robert's schema review | MOD-6 ext. 2026-07-14 (LXP D10); coupon action plan |

## C. Order lifecycle

| ID | Feature | Status | Source |
|---|---|---|---|
| C.1 | Order/OrderLine entities; no company columns (per-line company via resolved account) | Shipped | §4.2, MOD-3; S1 |
...
| G.3 | ScheduledJournalEntry bridge (dated entries created up-front at booking-lock) | Shipped ◇ | UPD-2; F4 |
| G.4 | Materialization of due scheduled entries (`Accounting.MaterializeDueScheduledEntries`) | Shipped ◇ (invokable core) / Planned (daily Scheduled Action seed) | B3.2 + DEFERRALS row |
| G.5 | Subscription lifecycle: find-or-extend-or-create on first Confirm | Deferred | BO-D40; DEFERRALS (F4 row) |
| G.6 | Renewal-order spawning (scheduled job; renewals default Draft) | Deferred | BO-D40; DEFERRALS (F4 row) |
...
| ID | Feature | Status | Source |
|---|---|---|---|
| K.1 | Order tax structure decision (Option A tax-line vs Option B durable shape) | Planned — HIGH, gated on Robert (Q21) | S4; DEFERRALS tax note (LXP D13 = real consumer + clock) |
| K.2 | OrderLineTaxLine per-jurisdiction breakdown + provider invocation at line time | Planned — behind K.1 | BO-D20; Phase G / F5 |
...
| ID | Feature | Status | Source |
|---|---|---|---|
| M.1 | Orders-seeded roles (order entry, Order Fulfiller) + RLS enforcement | Planned — co-design gated (accounting A2 / Q22 mechanism) | MOD-9a; F6; instance Task 32 |

## N. Consumers & integrations

| ID | Feature | Status | Source |
|---|---|---|---|
| N.1 | LXP (Sidecar) launch surface: LH4I individual checkout (tiers + coupons + Stripe upfront) | Planned — composite: B.3 + F.4 + K.1 + I.2 | UPD-6.1 (D1–D16) |
| N.2 | CDP data migration (legacy invoices → posted Orders, etc.) | Deferred — aidp Stage 4 | §13; DEFERRALS |
