...
> **Implements:** orders MOD-3 rev-2 (Order.CompanyID; line company FROM the product; resolution
...
`Subscription.OwningCompanyID` → `CompanyID` · new `OrderJournalEntry` (OrderID + JournalEntryID,
real FKs, unique pair) · `Order.JournalEntryID`: keep as DEPRECATED read-only during S1 (stamped
with the owning-company JE) so the UI tab doesn't break before S4 — removal is an S4-or-later item.
...
walk re-anchored: product link → category (route for the product's company) → PRODUCT-company
default → loud tripwire (unchanged) · hard-blocks per UPD-5: product-level GLAccountLink must
match the product's company (engine typed error + DB trigger floor, accounting repo); duplicate
(target × role × company) route rejected. *Demo artifact:* a multi-company order booked live —
