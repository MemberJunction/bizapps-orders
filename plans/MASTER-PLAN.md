...
| CA-3 | Backdating is allowed and "the only guard is a closed period" (07-09) — but accounting REMOVED `AccountingPeriod` (its MOD-1), so no closed-period notion exists to guard against. | **Resolved (final, Marcelo 2026-07-14 — after a same-day MOD-13 detour, withdrawn): NO closed-period guard.** Backdating ships unguarded; batch summaries lose date info anyway; accountants are responsible for batching entries into the right periods. Future timing complexities handled when they arise. | Marcelo 2026-07-14 (accounting MOD-1 final) ✅ |
