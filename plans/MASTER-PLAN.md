...
| CA-3 | Backdating is allowed and "the only guard is a closed period" (07-09) — but accounting REMOVED `AccountingPeriod` (its MOD-1), so no closed-period notion exists to guard against. | **Resolved-for-now: follow Amith's removal** (fuller verbatim in accounting MOD-1) — **backdating is allowed with NO guard for now**; batches land in the ERP's active period. Reopens only if Robert's ongoing research overturns the removal. | Marcelo 2026-07-13 (Amith doc confirmation) ✅(for now) |
