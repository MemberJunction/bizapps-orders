...
| B.1 | Pricing tables admin | Not started | Pricing admin screen (PriceList/ProductPrice/PriceTier) | — | wave §5.6 |
| B.2 | Price resolution in order entry (suggested price + source badge) | Not started | Compose Order line editor | — | UI plan §1 (F9-gated) · wave §5.6 |
...
| D.4–D.5 | Credit memo creation + settlement flows (reversal · refund · apply-credit) | Not started | reversal flow + shared application panel | — | UI plan §1/§4 · wave §5.10 (apply-credit) |
...
| F.4 | Stripe checkout (LXP subset) — PaymentIntent status + retry surfaces | Not started | payment rows + order Payments tab | — | F3.5b · wave §5.9 |
| F.7 | Payment application UI (+ auto-apply suggestion later) | Not started | Payments UI | — | UI plan §4 |
| F.9 | Payment-method vault management (masked list, default, remove; add = hosted tokenization) | Not started (add gated on F.4) | Customer A/R view + payment entry | — | wave §5.8 |
| F.10 | Webhook receiver | N/A — no UI | — | — | wave §5.9 note |
| F.11–F.13 | Gift card / providers / disputes | N/A — no UI (deferred features) | — | — | — |
| G.1–G.4 | Subscription + rev-rec schedule visibility | Not started | Subscriptions list + S8 waterfall viewer | — | wave §5.7 |
| G.7–G.8 | Overdue worklist + dunning grace display | Not started | Customer A/R view | — | UI plan §5 |
| H.1–H.4 | Sales rules admin + rule editor + approval flow surfacing | Not started | rule admin grid + form-based editor + S1 inbox + stepper state | — | wave §5.5 |
| I.1 | Entitlement grants visibility | Not started | grants grid (customer/product presets) | — | wave §5.11 |
