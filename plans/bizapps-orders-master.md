...
| D81 | **`bizapps-accounting` is a MANDATORY peer dependency.** Orders books revenue on confirm and cash on capture — `PaymentLineEntityServer` statically imports `AccountingEngineBase` and calls it on every allocation. There is no degraded mode and no feature flag. The `optional: true` markings were a workaround for accounting being unpublished (it resolves through a sibling-checkout symlink, and a CI runner cannot fetch it), not a design statement, and they have been removed. Until accounting publishes, a root `.npmrc` sets `legacy-peer-deps=true` so npm stops trying to fetch a peer that does not exist yet — the declaration stays mandatory and true. **Delete the `.npmrc` on publish day.** | Amith 2026-08-02: "accounting must definitely NOT be an optional peer dep, we must have a hard requirement for this — orders and payments both create JEs." Full rationale: [`docs/dependency-on-accounting.md`](../docs/dependency-on-accounting.md). |
