...
| D62 | **`SubscriptionType.BenefitModel` separates who HOLDS a subscription from who BENEFITS, and that sets the DEDUPE SCOPE.** `SubscriberScope` alone conflated the two. `Holder` = only the named holder benefits (individual membership). `OrganizationMembers` = the org holds it and its people benefit collectively (a trade association where a company joins and every employee is a member by virtue of that; no person is named). `NamedIndividual` = the org holds and PAYS while a NAMED person benefits (a corporate seat; both sides required). This is not classification for its own sake — it decides what counts as a duplicate: `OrganizationMembers` keys on the ORG so a second purchase extends the company's one membership, while `NamedIndividual` keys on the (org, person) PAIR so ten seats for ten staff are ten subscriptions rather than ten collisions under `RejectDuplicate`. **Before this, one order could not buy seats for more than one person.** | Amith 2026-07-26: "sometimes a sub accrues benefits to an org… some sub types can be org level and others indiv level and some might be mixed." |
